...
uint currentDirSectorAddr;
uint FATstartSector;
uint partition_firstSectorBlockAddr;
uint currentDirSectorNum;
...
uint FAT_setNextCluster(SDDataBlock_t dataBlock[], uint clusterNum, uint nextClusterNum, port p_sd_cmd, port p_sd_clk, port p_sd_dat)
{
  ...
  if ((clusterNum & 0x1) == 1)
  {
    ...
    dataBlock[0].data[wordAddr] = (dataBlock[0].data[wordAddr] & ((uint)0xFFFF<<16)) | (((nextClusterNum & 0xFF) << 8) | (((nextClusterNum>>8) & 0xFF)));
  }
  else
  {
    ...
    // set the next cluster
    dataBlock[0].data[wordAddr] = (dataBlock[0].data[wordAddr] & 0xFFFF) | (((nextClusterNum & 0xFF) << 24 )) | ((nextClusterNum & (uint)0xFF00) << 8);
  }
...
{
  uint newCluster;
  uint halfWordPointer;
  uint nextCluster;
  uint currentFATblock;
  ...
  {
    dataBlock[0].data[newCluster>>1] = (dataBlock[0].data[newCluster>>1] | ((uint) 0xFFFF<<16));
  }
...
uint FAT_readDirTableEntry(SDDataBlock_t dataBlock[], char fileName[], DIR_t dir[], port p_sd_cmd, port p_sd_clk, port p_sd_dat)
{
  ...
  uint clusterNum;
  uint fileSize;
  uint dirNameBuffPointer = 0;
  uint readNextEntry = TRUE;
  uint moreEntries = TRUE;
...
XMOS_RTN_t FAT_readPartitionTable(SDDataBlock_t dataBlock[], port p_sd_cmd, port p_sd_clk, port p_sd_dat)
{
    uint word;
    uint fileSystem;
    uint partition_numSectors = 0;
    uint partition_firstPartitionSector = 0;
    uint partition_lastPartitionSector = 0;
...
{
    uint numFATs;
    uint numRootDirEntries;
    uint numReservedSectors;
    uint numFATSectors;
    uint numRootDirSectors;
    uint sectorsPerFAT;
    uint partition_1stSectorBlockAddr = partition_firstSectorBlockAddr;
...
uint FAT_newFile(SDDataBlock_t dataBlock[], FP_t fp[], char filename[], port p_sd_cmd, port p_sd_clk, port p_sd_dat)
{
  uint directoryEntryAddr;
  uint clusterNum;
  uint word;
  uint dirSector;
  uint found;
...
//
// Return FALSE if the file could not be opened, or the file pointer initialised to
...
uint FAT_fread(SDDataBlock_t dataBlock[], FP_t fp[], char buf[], uint size, uint count, port p_sd_cmd, port p_sd_clk, port p_sd_dat)
{
  uint lastByteAddr;
  uint bufPointer;
  uint blockNextWordAddr;
  uint word;
  uint clusterSectorAddr;
  uint currentWord;
  uint startBytePos;
